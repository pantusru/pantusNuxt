    ...
    Panel: {
      ...
      set(value) {
        ...
        if (this.NameData !== undefined) {
          // Если ли потомки у панели
          this.checkChildrenData();
          if (length === 0) {
            // Проверяка что нужно удалить все выбранные потомки
            ...
            this.PanelSelectedChildren.forEach((SelectedChildren, index) => {
              let check = false;
              for (const dataChildren in this.PanelDataChildren) {
                // Прогоняем отображаемые потомки
                if (
                  SelectedChildren === this.PanelDataChildren[dataChildren].id
                ) {
                  // Выбранный потомок
                  check = true;
                  break;
              ...
              if (check === false || this.PanelDataChildren.length === 0) {
                // Прогоняем выбранные потомки
                this.$store.commit("Applicabilities/Panel/DeleteElementPanel", {

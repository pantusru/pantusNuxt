      ...
      <template v-for="table in array">
        <b-tr :key="table.ProductCard.id" class="hover-true">
          ...
          <b-td :rowspan="table.productOffer.length+1">
            <nuxt-link class="text-576b77 link-danger" :to="'product/' +table.ProductCard.id.toString()">
        ...
        <b-tr class="hover-true border-bottom" v-for="offer in table.productOffer" :key="offer.id">
          ...
          <b-td class="border-top-0 text-00b91e" :class="{'d-none d-md-table-cell': CheckAnalogs}">{{
              offer.quantity
            }}
          </b-td>

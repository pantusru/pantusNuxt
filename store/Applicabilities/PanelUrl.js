export const actions = {
  ...
  SetId_Url({ dispatch, commit }, data) {
    ...
    for (let keyData in data.data) {
      ...
      if (data.data[keyData].children.length !== 0) {
        // У данной приминимости есть Потомки?
        dispatch("SetId_Url", {
  ...
  MarkaSet({ rootState, commit }, data) {
    ...
    commit(
      "Applicabilities/Panel/SetPanelObject",
      {
  ...
  ModelSet({ rootState, commit, dispatch }, data) {
    ...
    for (const key in Panel) {
      // Прогоняем Panel и ищем совпадения с Маркой
    ...
    if (check === false) {
      // Все Panel проверены и был ли найден Panel с этим Id
      commit("Applicabilities/Panel/SetPanelNew", {}, { root: true });
      let Ids = rootState.Applicabilities.Panel.Ids;
      commit(
        "Applicabilities/Panel/SetPanelObject",
        {
          // Сохраняет  SelectedMarka SelectedModel DataGenerations DataModel
          id: Ids,
          name: [
            "SelectedMarka",
            "SelectedModel",
  ...
  GenerationsSet({ rootState, dispatch, commit }, data) {
    ...
    for (const keyPanel in Panel) {
      // Прогоняем Panel
      if (Panel[keyPanel].SelectedMarka == data.MarkId) {
        // Ищем совпадения с MarkId
        checkMark = true;
        for (const keyModel in Panel[keyPanel].SelectedModel) {
          // Прогоняем SelectedModel
          if (Panel[keyPanel]?.SelectedModel[keyModel] == data.link.parentId) {
            // Ищем совпадения с ModelId
            checkModel = true;
            commit(
      ...
      if (checkMark === true && checkModel === true) {
        // Проверяем были ли совпадения в checkMark
        break;
    ...
    if (checkMark === false) {
      // Пройдены все Panel и не найдены совпадения
      dispatch("MarkaSet", {
  ...
  PushPanelDataGenerations({ commit }, data) {
    let arr;
    if (data.linkPanel.DataGenerations.length === 0) {
    ...
    commit(
      "Applicabilities/Panel/SetPanel",
      {
